...
namespace {
    ...
    TEST(SumTestsVector, Normal) {
        std::vector<int> inputs = {1, 3, 5};
        Result result = findSum(inputs, 6);
        EXPECT_EQ(0, result.startIndex);
        EXPECT_EQ(2, result.endIndex);
    }

    TEST(SumTestsVector, TargetSmallerThanFirst) {
        std::vector<int> inputs = {1, 3, 5};
        Result result = findSum(inputs, 0);
        EXPECT_EQ(-1, result.startIndex);
        EXPECT_EQ(-1, result.endIndex);
    }

    TEST(SumTestsVector, TargetSmallerThanSecond) {
        std::vector<int> inputs = {1, 3, 5};
        Result result = findSum(inputs, 3);
        EXPECT_EQ(-1, result.startIndex);
        EXPECT_EQ(-1, result.endIndex);
    }
}